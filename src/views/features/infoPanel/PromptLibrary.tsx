...
export function PromptLibrary({ setPromptTemplate }: PromptLibraryProps) {
  return (
    <ul
      role="list"
      className="list-none p-0 grid grid-cols-1 -m-2 sm:grid-cols-2 lg:grid-cols-3"
    >
      {prompts.map(({ icon: Icon, title, template }) => (
        <li
          key={title}
          className="m-2 col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow"
        >
          <button
            type="button"
  ...
  return (
    <div className="relative">
      ...
      <div
        className={
          open
            ? "visible absolute left-0 bottom-10"
            : "invisible absolute left-0 bottom-10 "
        }
      >
        <ul
